...
public class ActiveSearchPage
{
    ...
    private static readonly Template<SearchResult> RenderSearchResult = (page, result) => page.Html($"""
         <tr>
             <td>{result.Text}</td>
         </tr>
         """);

    private static readonly Template Empty = (page) => page.Html($"""<div id="results"></div>""");

    private static readonly Template<string> RenderResults = (page, query) => page.Html($"""
        <table id="results">
    ...
    private static readonly Template<string?> Page = (page, query) => page.Html($"""
         ...
         <active-search>
          ...
          {(query, (p, q) => !string.IsNullOrWhiteSpace(q)
                ? RenderResults(p, q)
                : Empty(p))}
         """);
